...
io.sockets.on('connection', function(socket) {
    ...
    if(data.format==="ba") args.push(`-P ${downloadpath ||"public/audio/"}`),args.push(`-f ${data.format}`)
    if(data.format.split(" ").length===2) {args.push(data.format.split(" ")[0]),args.push(data.format.split(" ")[1]),args.push(`-P ${downloadpath || "public/thumbnail/"}`)}
